...
#### FIDO U2F Tokens and USB devices, and power saving
echo -e "${GREEN}Udev rules for FIDO, USB, and power saving${NOCOLOR}" && sleep 1
cp $SCRIPTPATH/conf/ubuntu/udev/*.rules /etc/udev/rules.d/
...
# Utilities
apt-get install -y \
	aptitude screen tmux htop iotop iftop acct smartmontools molly-guard curl openssh-blacklist openssh-blacklist-extra \
	apparmor apparmor-profiles apparmor-profiles-extra \
